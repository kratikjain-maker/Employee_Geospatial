# ============================================================================
# 🚀 STREAMLIT EMPLOYEE GEOSPATIAL ANALYTICS DASHBOARD
...
def load_and_process_data(uploaded_file):
    ...
    def get_direction(lat, lon):
        ...
        if -22.5 <= angle < 22.5: return 'N'
        elif 22.5 <= angle < 67.5: return 'NE'
        elif 67.5 <= angle < 112.5: return 'E'
        elif 112.5 <= angle < 157.5: return 'SE'
        elif angle >= 157.5 or angle < -157.5: return 'S'
        elif -157.5 <= angle < -112.5: return 'SW'
        elif -112.5 <= angle < -67.5: return 'W'
        else: return 'NW'
    
    df_valid['Direction'] = df_valid.apply(lambda r: get_direction(r['Lat'], r['Lon']), axis=1)
    
    # Clustering
    coords = df_valid[['Lat', 'Lon']].values
    dbscan = DBSCAN(eps=0.01, min_samples=5).fit(coords)
    df_valid['Cluster'] = dbscan.labels_
    
    kmeans = KMeans(n_clusters=min(8, len(df_valid)), random_state=42).fit(coords)
    df_valid['Zone'] = kmeans.labels_
...
def main():
    # Header
    st.markdown('<h1 class="main-header">🗺️ Employee Geospatial Analytics Dashboard</h1>', 
                unsafe_allow_html=True)
    ...
    # Sidebar
    with st.sidebar:
        st.image("https://img.icons8.com/color/96/000000/google-maps-new.png", width=80)
        ...
        if uploaded_file is not None:
            ...
            # We'll add filters after loading data
        else:
    ...
    with st.sidebar:
        ...
        max_distance = st.slider(
            "Max Distance (km)",
            min_value=0,
            max_value=int(df_valid['Distance_km'].max()) + 1,
            value=int(df_valid['Distance_km'].max()) + 1,
            help="Filter employees by distance from office"
        )
        
        # Department filter
        departments = ['All'] + sorted(df_valid['Department'].unique().tolist())
        selected_dept = st.selectbox("Department", departments)
        
        # Grade filter
        grades = ['All'] + sorted(df_valid['Grade'].unique().tolist())
        selected_grade = st.selectbox("Grade", grades)
        
        # Direction filter
        directions = ['All'] + sorted(df_valid['Direction'].unique().tolist())
    ...
    with col1:
        st.metric("👥 Total Employees", f"{len(filtered_df):,}", 
                 f"{len(filtered_df) - len(df_valid):+,}" if len(filtered_df) != len(df_valid) else None)
    
    with col2:
        st.metric("📍 Unique Regions", f"{filtered_df['Region'].nunique()}")
    
    with col3:
        st.metric("📏 Avg Distance", f"{filtered_df['Distance_km'].mean():.2f} km")
    
    with col4:
        within_10km = (filtered_df['Distance_km'] <= 10).sum()
        pct = within_10km / len(filtered_df) * 100 if len(filtered_df) > 0 else 0
        st.metric("Within 10km", f"{within_10km:,}", f"{pct:.1f}%")
    
    with col5:
        st.metric("🎯 Clusters Found", f"{len(set(filtered_df['Cluster'])) - (1 if -1 in filtered_df['Cluster'].values else 0)}")
    ...
    with tab1:
        ...
        with col2:
            ...
            show_clusters = st.checkbox("Show Cluster Boundaries", value=False)
        ...
        region_stats_filtered = region_stats[region_stats['Region'].isin(filtered_df['Region'].unique())]
        ...
        fig_regions = px.scatter_mapbox(
            ...
            height=500,
            mapbox_style=map_style,
            title=f"Top 20 Regions by Employee Count"
        )
        
        fig_regions.update_layout(mapbox_center={'lat': OFFICE_LAT, 'lon': OFFICE_LON})
    ...
    with tab2:
        ...
        dist_bins = pd.cut(filtered_df['Distance_km'], bins=[0, 5, 10, 15, 20, 100])
        dist_counts = dist_bins.value_counts().sort_index()
        fig_dash.add_trace(
            go.Bar(x=[str(x) for x in dist_counts.index], y=dist_counts.values, 
                   marker_color='#1e88e5', showlegend=False),
            row=1, col=1
        )
        ...
        fig_dash.add_trace(
            go.Pie(labels=dir_counts.index, values=dir_counts.values, hole=0.4, showlegend=False),
            row=1, col=2
        )
        ...
        fig_dash.add_trace(
            go.Bar(y=dept_counts.index, x=dept_counts.values, orientation='h',
                   marker_color='#43a047', showlegend=False),
            row=2, col=1
        )
        ...
        fig_dash.add_trace(
            go.Pie(labels=grade_counts.index, values=grade_counts.values, hole=0.3, showlegend=False),
            row=2, col=2
        )
        ...
        fig_dash.add_trace(
            go.Scatter(x=sorted_dist, y=cumulative, mode='lines', 
                       line=dict(color='#e53935', width=3), showlegend=False),
            row=3, col=1
        )
        ...
        fig_dash.add_trace(
            go.Bar(y=top_regions.index, x=top_regions.values, orientation='h',
                   marker_color='#fb8c00', showlegend=False),
            row=3, col=2
        )
    ...
    with tab3:
        ...
        st.markdown("*GPU-accelerated visualization showing employee concentration as building heights*")
        ...
        hex_layer = pdk.Layer(
            ...
            coverage=0.95,
            radius=500,
            get_fill_color='[255, 140 * (1 - elevationValue / 1000), 0]',
            pickable=True
        )
        ...
        st.info("💡 **Tip:** Use mouse to rotate and tilt the 3D view. Taller hexagons indicate higher employee density.")
    ...
    with tab4:
        ...
        with col1:
            ...
            avg_dist = filtered_df['Distance_km'].mean()
            max_dist = filtered_df['Distance_km'].max()
            within_5km = (filtered_df['Distance_km'] <= 5).sum()
            within_10km = (filtered_df['Distance_km'] <= 10).sum()
            beyond_15km = (filtered_df['Distance_km'] > 15).sum()
            
            st.markdown(f"""
            1. **Distance Analysis:**
               - Average commute: **{avg_dist:.2f} km**
               - Maximum distance: **{max_dist:.2f} km**
               - Within 5km: **{within_5km}** ({within_5km/len(filtered_df)*100:.1f}%)
               - Within 10km: **{within_10km}** ({within_10km/len(filtered_df)*100:.1f}%)
               - Beyond 15km: **{beyond_15km}** ({beyond_15km/len(filtered_df)*100:.1f}%)
            ...
            3. **Natural Clusters:**
               - Identified **{len(set(filtered_df['Cluster'])) - (1 if -1 in filtered_df['Cluster'].values else 0)}** distinct employee clusters
        ...
        with col2:
            ...
            if beyond_15km > 0:
                recommendations.append(
                    f"🚌 **Shuttle Service**: {beyond_15km} employees ({beyond_15km/len(filtered_df)*100:.1f}%) live beyond 15km. "
                    f"Consider implementing shuttle routes for these areas."
                )
            
            top_region = filtered_df['Region'].value_counts().head(1)
            if len(top_region) > 0 and top_region.values[0] > 30:
                recommendations.append(
                    f"🏢 **Satellite Office**: {top_region.index[0]} has {top_region.values[0]} employees. "
                    f"Evaluate feasibility of a satellite office or co-working space partnership."
                )
            
            if (filtered_df['Distance_km'] > 20).sum() > 10:
            ...
            clusters = len(set(filtered_df['Cluster'])) - (1 if -1 in filtered_df['Cluster'].values else 0)
            if clusters >= 3:
                recommendations.append(
                    f"🎯 **Housing Partnerships**: {clusters} natural clusters identified. "
        ...
        with col1:
            ...
            top_regions_df = filtered_df['Region'].value_counts().head(10).reset_index()
        ...
        with col2:
            ...
            dir_df['Percentage'] = (dir_df['Employee Count'] / len(filtered_df) * 100).round(1)
    ...
    with tab5:
        ...
        with col1:
            ...
            st.caption(f"Includes {len(filtered_df)} employees with distance, direction, and cluster data")
        
        with col2:
            st.markdown("### 🌍 Regional Statistics")
            region_export = region_stats[region_stats['Region'].isin(filtered_df['Region'].unique())].copy()
            ...
            st.caption(f"Regional aggregations and statistics")
        ...
        with col3:
            ...
            report = {
                ...
                'summary': {
                    ...
                    'avg_distance_km': float(filtered_df['Distance_km'].mean()),
                    'max_distance_km': float(filtered_df['Distance_km'].max()),
                    'within_10km': int((filtered_df['Distance_km'] <= 10).sum()),
                    'within_10km_pct': float((filtered_df['Distance_km'] <= 10).sum() / len(filtered_df) * 100)
                },
                'top_regions': region_export.head(10).to_dict('records'),
                'directional_distribution': filtered_df['Direction'].value_counts().to_dict()
            }
        ...
        st.info("💡 **Tip:** You can download any chart as PNG by hovering over it and clicking the camera icon in the top-right corner.")
